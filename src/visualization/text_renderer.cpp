...
TextRenderer::TextRenderer(std::shared_ptr<FontManager> fontManager,
    ...
    : fontManager_(fontManager), shaderManager_(shaderManager), impl_(std::make_unique<Impl>()) {
}
...
bool TextRenderer::initialize(RenderMode mode) {
    ...
    if (!fontManager_) {
        std::cerr << "Error: FontManager is null in TextRenderer::initialize" << std::endl;
        return false;
    }

    if (!shaderManager_) {
        std::cerr << "Error: ShaderManager is null in TextRenderer::initialize" << std::endl;
        return false;
    }

    if (!impl_) {
        std::cerr << "Error: Implementation (impl_) is null in TextRenderer::initialize"
                  << std::endl;
        return false;
    }
    ...
    if (glfwGetCurrentContext() == nullptr) {
        std::cerr << "Error: No current OpenGL context in TextRenderer::initialize" << std::endl;
        return false;
    }
    ...
    if (!shaderManager_->createShaderProgram(impl_->shaderName, textVertexShaderSource,
...
void TextRenderer::drawText(const std::string& text, float x, float y, const Vector4& color) {
    ...
    Matrix4 projection = glm::ortho(-halfWidth, halfWidth, -halfHeight, halfHeight, -1.0f, 1.0f);
