...
bool FontManager::initialize(RenderMode mode) {
  ...
  // Load fonts
  if (!loadSystemFonts()) {
    std::cerr << "Failed to load any system fonts. Creating fallback font."
              << std::endl;
    createFallbackFont();
  }
...
// Platform-specific font paths
  ...
  const char *fontPaths[] = {"/System/Library/Fonts/HelveticaNeue.ttc",
...
bool FontManager::isInitialized() const { return impl_->initialized; }
